...
def main():
    ...
    <div style='font-style: italic; font-weight: bold; font-size: 15px; color: #222222; border-left: 3.5px solid gray; padding-left: 10px; margin-bottom: 10px;'>
        💡"aaj kamayega toh kal khayega apni mehnt par bharosa rakh <br> form 2 tho dobara bhi ho jaega" <br> -Sameer🌳
    </div>
    """
...
def extract_project_name():
    global formatted_text  # Access the global variable
    project_name_text = \
        formatted_text.split("Certificate|of|Financial|Progress|of|Work|of|")[1].split("|having|MahaRERA|Registration|")[
            0].strip()
...
def extract_registration_number():
    global formatted_text  # Access the global variable
    registration_number = formatted_text.split("|MahaRERA|Registration|Number")[1].split("|being|developed|by")[0].strip()
    return registration_number

def extract_ECC():
    global formatted_text  # Access the global variable
    ECC_text = formatted_text.split("a|Estimated|Cost|of|Construction|as|certified|by|Engineer.|")[1].split("b.|")[0].strip()
